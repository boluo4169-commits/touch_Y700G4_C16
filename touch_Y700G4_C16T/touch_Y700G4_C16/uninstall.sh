...
[ -f "$MODDIR/perf.pid" ] && kill "$(cat "$MODDIR/perf.pid")" 2>/dev/null
for pid in $(ls /proc | grep -E '^[0-9]+$'); do
    case "$(cat /proc/$pid/cmdline 2>/dev/null | tr '\0' ' ')" in
        *touch_daemon.sh*|*perf_daemon.sh*) kill -9 "$pid" 2>/dev/null ;;
    esac
done
echo "已停止后台守护"
...
for c in 0 1 2 3 4 5 6 7; do
    ...
    [ -n "$HWMAX" ] && echo "$HWMAX" > "$D/scaling_max_freq" 2>/dev/null
done
echo "触控节点已复位, CPU频率已还原"

# ============================================================
# 4. 清除运行时文件 (log/pid)  [模块文件本身由KSU删除]
# ============================================================
rm -f "$MODDIR/apply.log" "$MODDIR/perf.log" 2>/dev/null
rm -f "$MODDIR/daemon.pid" "$MODDIR/perf.pid" 2>/dev/null
echo "运行时文件已清除"
