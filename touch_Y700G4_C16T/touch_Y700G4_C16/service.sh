...
echo "$(date): ========== touch_Y700G4_C16 v3.0 start (fps=${TARGET_FPS}) ==========" > "$LOG_FILE"
...
apply_touch_tuning() {
    ...
    if [ -f /proc/game_mode ]; then
        # v2.9: 开机只读不写, game_mode 完全交给系统管理
...
echo "$(date): 触控守护已脱离启动 PID=$!" >> "$LOG_FILE"

# ============================================================
# [合并] 拉起"负载+温度自适应"性能守护
# 注意: KSU flat安装后脚本权限可能丢x位, 这里主动chmod确保可执行
# ============================================================
PERF_DAEMON="$MODDIR/perf_daemon.sh"
if [ -f "$PERF_DAEMON" ]; then
    chmod 755 "$PERF_DAEMON" 2>/dev/null
    sh "$PERF_DAEMON" stop 2>/dev/null
    # 用 setsid 完全脱离本会话启动, 防止随父shell退出被回收
    setsid nohup "$PERF_DAEMON" start >/dev/null 2>&1 &
    echo "$(date): 性能自适应守护已拉起(PID=$! 权限已补)" >> "$LOG_FILE"
fi